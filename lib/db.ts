...
export const knowledgeDocuments = pgTable(
  ...
  (table) => [
    // Index on source document for fast filtering
    index('knowledge_source_idx').on(table.sourceDocument),
    // Index on tenant_id for tenant-scoped queries (every retrieval query uses this)
...
export const conversationMessages = pgTable(
  ...
  (table) => [
    ...
    index('conv_seq_idx').on(table.conversationId, table.sequence),
  ],
);
...
export const agentTraces = pgTable(
  ...
  (t) => ({
    ...
    toolCalls: t.jsonb('tool_calls').$type<
      Array<{
        tool: string;
    ...
    retrievedContext: t.jsonb('retrieved_context').$type<
      Array<{
        chunkId: string;
        score: number;
        content: string;
    ...
    guardrailChecks: t.jsonb('guardrail_checks').$type<
      Array<{
        rule: string;
        passed: boolean;
        details: string;
  ...
  (table) => [
    ...
    index('trace_conv_idx').on(table.conversationId),

    // Index for filtering by outcome (e.g., "show me all escalations")
    index('trace_outcome_idx').on(table.outcome),

    // Index for time-range queries ("traces from the last hour")
    index('trace_time_idx').on(table.createdAt),
  ],
);
